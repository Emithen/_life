...
- 🎯 다음: 기기 동기화(Supabase, L5) → 아래 "국면 B로 가기 전에 정리된 것"
...
## 국면 B(기기 동기화)로 가기 전에 정리된 것 (2026-08-02)
...
|---|---|
| "호스팅을 Pages 대신 Vercel/Netlify로 옮겨야" | **필요 없다.** `supabase-js`는 GitHub Pages에서 그대로 돈다. anon key는 공개 전제이고 RLS가 보호한다. **제일 무거워 보이던 항목이 애초에 없는 항목이었다** |
| "collect.py가 Actions에서 Supabase에 써넣음" | `collect.py`는 07-31에 삭제됐다. **죽은 계획** |
...
| 안 | 드는 일 | 왜 안 골랐나 / 골랐나 |
|---|---|---|
| Google Drive `appDataFolder` | **제일 적다.** 이미 있는 OAuth에 스코프 한 줄 | 배우는 게 없다. SQL·RLS를 안 만난다. 확장도 막힌다 |
...
- ⚠️ **미검증**: Supabase가 구글 provider token을 자동 갱신해주는지. 안 되면 앱 데이터는 Supabase,
  구글 토큰은 지금 GIS 방식을 그대로 병행한다(버튼은 하나로 묶을 수 있다). **0단계에서 이걸 먼저 확인한다.**
- ⚠️ 무료 프로젝트는 **7일 무활동 시 일시정지**된다.

### 진짜 비용은 벤더가 아니라 `storage.js`가 **동기**라는 것 ⭐
...
→ **대응: localStorage를 버리지 말고 캐시로 남긴다.** 읽기는 지금처럼 로컬에서 동기로,
쓰기는 로컬에 먼저 쓰고 서버로 밀어올린다. 열 때 한 번 서버에서 당겨와 로컬을 덮는다.
동기 API가 그대로 살고 오프라인에서도 돈다. `channelCache`로 이미 해본 캐시-우선 패턴의 재사용이다.
...
2. Supabase 0단계 — 프로젝트 생성 → Google provider → 로그인해서 `user.id` 확인 (+ provider token 갱신 검증)
