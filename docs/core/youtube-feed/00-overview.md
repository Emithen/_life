...
- 🎯 **하기로 한 것** (2026-07-30 확정, ROADMAP 참고):
  1. **내 재생목록에서 랜덤 추천** — 유튜브 전용 재생목록(일부공개)을 Worker가 Data API로
     전부 읽어와 무작위 1개 제시. 본 영상은 후보에서 제외
     ⛔ 유튜브 "나중에 볼"(WL)은 2016년부터 API·RSS 모두 차단 — 대신 전용 재생목록을 쓴다
  2. 내보내기/가져오기 — 짧고, 3번의 데이터 이민 수단이 됨
  3. **로그인(국면 B, Supabase)** — 기기 간 동기화. L4 → **L5로 올라가는 지점**
