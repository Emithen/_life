...
- ✅ **내보내기/가져오기** (2026-08-02) — 백업 파일 한 장. `storage.js`의 `exportAll`/`importAll`
...
- `appDataFolder`는 사람 눈에 **안 보인다** → 직접 열어 고칠 수 없다(백업 파일이 필요한 또 하나의 이유)
...
2. **0단계**: Drive API 활성화 + 스코프 `drive.appdata` 추가 → `files.list`로 appData 폴더를 읽어
   **빈 목록이라도 200이 오는 것**까지 확인
3. **수동 동기화**: `☁️ 올리기`/`☁️ 내려받기` 버튼 — 백업 파일 자리에 Drive를 끼운다.
   `exportAll`/`importAll` 그대로 재사용 → **여기까지가 보상의 대부분**
...
⚠️ 브라우저 데이터를 지우면 전부 사라진다. 계정이 없으므로 **백업 파일이 유일한 복구 수단이다**(2026-08-02).
→ 화면의 `💾 백업`에서 내보내기. `channelCache`만 빠지고 나머지 4개가 파일 한 장에 담긴다.
