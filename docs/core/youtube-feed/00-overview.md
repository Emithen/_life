...
- ✅ **링크 공유 카드** (2026-07-29) — OG 태그 + `og-image.png`(원본 `og-image.svg`), 파비콘
- ✅ **본 영상 표시** (2026-07-29) — 클릭 시 자동 기록 + `✓`/`○` 수동 토글, 흐리게. 채널당 6개
- ✅ **NEW 배지** (2026-07-29) — 24시간 이내 업로드. 본 영상이면 배지를 감춤(본 것 우선)
- 🔜 다음 후보: 통합 타임라인(전체 최신순)·주제 필터, 썸네일, 내보내기/가져오기,
